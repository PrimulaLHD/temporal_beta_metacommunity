...
beta_temporal_metacomm <- function(m1,m2,method="Jac"){
  ...
  if(all(dim(m1)==dim(m2))){
    ...
    D0 <- m1_00-m2_00
    b0 <- length(D0[D0==1]) # unique numbers of item in m1, local-extinction driven part
    c0 <- length(D0[D0==-1]) # unique numbers of item in m2, local-colonization driven part
    
    # part 2, regional species changes
    
    R_mat_loss <- D[rowSums(m2)>0,colSums(m2) == 0]
    b_R <- length(R_mat_loss[R_mat_loss == 1]) # regional species loss (extinction) driven part
    
    
    R_mat_gain <- D[rowSums(m1)>0,colSums(m1) == 0]
    c_R <- length(R_mat_gain[R_mat_gain == -1]) # regional species gain (colonization) driven part
    ...
    if(method=="Jac"){
      ...
      beta_temporal <- ((b+c)/(a+b+c)) # total temporal beta diversity of link types
    ...
    if(method=="Sor"){
      ...
      beta_temporal <- ((b+c)/(2*a+b+c)) # total temporal beta diversity of link types
  ...
  res <- data.frame(Component=c("beta_Local","beta_Regional", "beta_Landscape",
                                "beta_RL", "beta_temporal","beta_extinction",
                                "beta_colonization"),
                    beta=c(beta_Local,  beta_Regional, beta_Landscape,
                           beta_RL,beta_temporal,beta_extinction, 
                           beta_colonization))
...
for (rep in 1:30) {
  load(paste("Data/data2/meta_df_eq_narrow_", rep,".Rdata",sep = ""))
  #meta_df_eq_narrow <- do.call("rbind", meta_df_eq_narrow)
